...
class AdversarialAttackerContainer:
    ...
    def train_ai_jamming(self, env: AdversarialAttackerEnv, act: AdversarialAttackerAct, is_ai_jam_enabled: bool) -> AdversarialAttackerAct:
        if self.isInference:
            return self.inference_ai_jamming(env, act, is_ai_jam_enabled)
        ...
        current_state = [env.throughput, self.target_throughput]
        ...
        reward = 10 if env.throughput < self.target_throughput else 0
    ...
    def inference_ai_jamming(self, env: AdversarialAttackerEnv, act: AdversarialAttackerAct, is_ai_jam_enabled: bool) -> AdversarialAttackerAct:
        # current_state = [env.throughput, self.target_throughput, env.mcs, self.attack_packet_count, env.snr]
        current_state = [env.throughput, self.target_throughput]
        if is_ai_jam_enabled == True:
...
if __name__ == '__main__':
    ...
    for _ in range(sim_runs):
        ...
        exp = Experiment(mempool_key, mem_size, 'ns3-sim', '../../', using_waf=False)
        exp.reset()
        c = AdversarialAttackerContainer(memblock_key, 
                                         model_name='',
                                         tpt_threshold=25, 
                                         retrain=True)
        ...
        epochs, isTqdm = 30000, True
        progress = tqdm(total=epochs) if isTqdm else None
        is_ai_jam_enabled = True

        while not c.rl.isFinish():
            with c.rl as data:
                if data is None:
                    break
                data.act = c.train_ai_jamming(data.env, data.act, is_ai_jam_enabled)
                if isTqdm:
                    progress.update(1)
                if iters_count > epochs:
                    c.save_model('test.ckpt')
                    print('Model saved')
                    break
                iters_count += 1
